...
export default async function WordSearch({word}:{word:string}){
    ...
    async function Posts(){
        const words = (await findWord(word)).Posts
        if (words.length > 0 ){
            ...
            return <div className="w-full h-full justify-center items-center"><h1 className="text-text">No matching word was found. You can add <Link href='/new-word'>{word}</Link></h1></div>
        }
    }
